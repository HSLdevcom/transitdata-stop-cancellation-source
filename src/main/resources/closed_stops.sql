SELECT SD.stop_deviations_id, SD.status, SD.valid_from AS SD_VALID_FROM, SD.valid_to AS SD_VALID_TO,
    ...
    FROM omm_db.dbo.stop_deviations AS SD
    LEFT JOIN omm_db.dbo.stop_controls AS SC ON SD.stop_deviations_id = SC.stop_deviations_id
    LEFT JOIN omm_db.dbo.bulletins B ON SD.bulletin_id = B.bulletins_id
    LEFT JOIN omm_db.dbo.bulletin_localized_messages AS BLM ON B.bulletins_id = BLM.bulletins_id
    LEFT JOIN omm_db.dbo.deviation_cases AS DC ON B.bulletins_id = DC.bulletin_id
