...
public class DoiAffectedJourneySource {
    ...
    private DoiAffectedJourneySource(PulsarApplicationContext context, Connection connection,
        ...
        queryString = QueryUtils.createQuery(getClass(),
                useTestDoiQueries
                        ? "/affected_journeys_by_journey_patterns_test.sql"
                        : "/affected_journeys_by_journey_patterns.sql");
    ...
    public Map<String, List<Journey>> queryByJourneyPatternIds(Collection<String> affectedJourneyPatternIds)
            throws SQLException {
        ...
        String preparedString = queryString.replace("VAR_FROM_DATE", dateFrom).replace("VAR_TO_DATE", dateTo)
    ...
    private Map<String, List<Journey>> parseJourneys(ResultSet resultSet) throws SQLException {
        ...
        for (String journeyPatternId : map.keySet()) {
            log.info("Found {} affected journeys for affected journey pattern id {}", map.get(journeyPatternId).size(),
                    journeyPatternId);
        }
        return map;
    }
